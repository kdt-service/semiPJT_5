...
#NanumGothic.ttf를 설정해놓는 경로. (그래프 한글 깨짐 방지, 한글이 깨지는 그래프마다 달아주어야함.)
fontprop = fm.FontProperties(fname=font_path)
#fontprop을 설정하는 이유: streamlit을 사용할 때 다른 사람들이 볼 수 있게 공유한다면 그래프에서 한글 파일이 깨져서 나옵니다.
#그래서 글씨체.ttf파일의 경로를 설정해두고 변수로 설정하여 그래프마다 넣어줘야합니다.

# requirements.txt는 깃허브에 연동하여 웹에 배포하기 위해서 설치해두면 좋습니다.
